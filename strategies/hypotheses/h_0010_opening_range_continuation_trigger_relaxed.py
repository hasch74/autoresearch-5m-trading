"""h_0010 — Opening-range continuation with relaxed trigger gates.

Variant of h_0007:
  - Only breakout/extension trigger strictness is relaxed.
  - RVOL and context gates remain unchanged.
"""
...
class OpeningRangeContinuationTriggerRelaxed:
    """First-hour continuation with relaxed breakout trigger strictness."""

    name: str = "opening_range_continuation_trigger_relaxed"
    version: str = "0.1"
    hypothesis_id: str = "h_0010"

    min_minutes_since_open: int = 25
    max_minutes_since_open: int = 55
    min_rvol: float = 1.4
    min_breakout_buffer_atr: float = 0.03
    max_extension_above_or_atr: float = 0.9
